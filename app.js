...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (typeof pdfjsLib !== "undefined") {
    // Force pdf.js v3 into "fake worker" mode (main-thread parsing).
    // Customs declarations are 1-5 pages so main-thread is <100 ms.
    // This avoids ALL corporate proxy / Tracking Prevention / CSP issues.
    //
    // Strategy:
    //  1. pdf.worker.min.js is loaded as a static <script> tag in HTML
    //     (both index.html and ImportHRO.html), setting window.pdfjsWorker.
    //  2. We inject WorkerMessageHandler into PDFWorker so _setupFakeWorker
    //     returns it immediately — no dynamic loadScript() call at all.
    //  3. Override window.Worker to force the fake-worker fallback path.
    //  4. workerSrc is set non-empty to pass pdf.js v3 validation.
    ...
    // Inject the pre-loaded WorkerMessageHandler so _setupFakeWorkerGlobal
    // returns it directly without calling loadScript().
    var workerModule = (typeof window !== "undefined" && window.pdfjsWorker) ||
                       (typeof globalThis !== "undefined" && globalThis.pdfjsWorker);
    if (workerModule && workerModule.WorkerMessageHandler) {
      pdfjsLib.PDFWorker._mainThreadWorkerMessageHandler = workerModule.WorkerMessageHandler;
      console.log("[app.js] pdfjsLib version:", pdfjsLib.version, "(main-thread mode, WorkerMessageHandler injected)");
    } else {
      console.warn("[app.js] pdfjsLib version:", pdfjsLib.version, "(fake-worker mode, pdfjsWorker NOT pre-loaded — will try loadScript)");
    }
  }
  setupDropZone();
  setupButtons();
...
async function extractPdfText(file, onPageProcessed) {
  ...
  try {
    ...
    diagLog("Step 2/3: Opening PDF with pdf.js (fake-worker / main-thread)...");
